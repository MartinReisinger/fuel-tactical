...
# Flask's static_folder=FRONTEND_DIR with static_url_path='' means:
#   GET /css/style.css      → frontend/css/style.css
#   GET /js/map.js          → frontend/js/map.js
#   GET /i18n/translations.js → frontend/i18n/translations.js
# No extra routes needed for these.
...
def stations():
    ...
    if needs_update:
        ...
        try:
            r = requests.get(
                url,
                params={"latitude": lat, "longitude": lon, "fuelType": fuel, "includeClosed": "false"},
                headers={"User-Agent": "FuelTactical/1.0"},
                timeout=5
            )
